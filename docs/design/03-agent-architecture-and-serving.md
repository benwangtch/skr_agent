...
`serving/a2a.py` 把 skr agent 包成一個講 A2A（JSON-RPC + REST）的 FastAPI app：
...
**接線方式**（`a2a-sdk` 1.1.x 的實際 API，不是網路教學常見的 `A2AStarletteApplication`——那個 class 這個版本沒有，是直接 import 套件用 `inspect.signature()` 逐一核對出來的）：

- `LegacyRequestHandler`（不是文件常見的 `DefaultRequestHandler`）+ `InMemoryTaskStore` + `agent_card`
- `create_agent_card_routes()` / `create_jsonrpc_routes()` / `create_rest_routes()` 產生路由，`add_a2a_routes_to_fastapi()` 掛進 FastAPI app
...
SendMessage 進來
  ...
  → response.ok ? updater.complete(...) : updater.failed(...)
```
...
**協定版本限制（實測，非文件記載）**：每個請求都必須帶 `A2A-Version: 1.0` header，沒帶會被拒絕（`VERSION_NOT_SUPPORTED`），`enable_v0_3_compat=True` **不會**放寬這個限制。細節見 `01-config-and-serving.md` §4、`RUNBOOK.md` §3.6。

**刻意不做的兩件事**：

1. **Streaming task events。** 每次呼叫都是跑完整個 agent 才一次性回一則訊息。要接 `TaskArtifactUpdateEvent` 增量更新的話，`DeepAgent.run()` 要先能吐出部分輸出——LangGraph 本身支援 `astream`，所以這件事現在比之前好做，但還沒做。
2. **真的驗證呼叫方身份。** `PrincipalResolver` 就是那個縫。
...
測試套件（129 個，全部不需要金鑰）測的是接縫，不是模型。所以換框架後另外用一個**本地假 OpenAI-compatible server**（回一次 tool call、再回一次最終答案）把整條鏈路跑過一遍，確認：
...
- A2A JSON-RPC round trip 回 `TASK_STATE_COMPLETED`
...
3. **A2A 沒有 streaming。** LangGraph 支援 `astream`，所以現在比之前好接，但還沒接。
4. **`cost_usd` 永遠是 0。** 舊 framework 的 `ResultMessage` 會回報成本，LangChain 的 `usage_metadata` 只有 token 數。要算成本得自己維護一份各 provider 的價目表——與其塞一個會過期的錯數字，不如先誠實回報 0，token 數是準的。
5. **deepagents 內建的 filesystem / `execute` tools 沒有明確限制範圍。** 目前用預設的 `StateBackend`（虛擬、記憶體內，不碰真實磁碟），所以 agent 的檔案操作是沙盒化的；但這是靠「沒去改預設值」得到的，不是明確設定的。如果之後有人為了讓 skill 從磁碟載入而改成 `FilesystemBackend`，agent 就會取得專案目錄的實際讀寫權限——那時候要一併設 `permissions=[FilesystemPermission(...)]`，不能只換 backend。
6. **多層委派（A→B→C）沒驗證過。** `agent_as_tool` 的 budget 傳遞、trace 追蹤、citation 彙總目前只被單層委派驗證過。
